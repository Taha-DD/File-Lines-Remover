...
int main() {
    FILE *file, *f;
    char *line, *l; // buffers to move and copy lines in the file
    ...
    long len = 0; // taking line lingth from getline
    ...
    while (getline(&line, &len, file) >= 0) { //moving to the lines in the file
        ...
        if (count > StartDelete) {
            ...
            if(getline(&l, &len, f) >= 0);//moving to the next line we wanna overwrite with
            else break;
        }
    }
